...
启动时可选加载 TOML 配置文件：路径经环境变量 `CRED_HUB_CONFIG` 指定，默认读当前目录的 `cred-hub.toml`，文件不存在则全部用默认值（解析出错会直接启动失败，不静默）。**优先级：环境变量 > 配置文件 > 内置默认值**。

配置文件结构与 CF 版 [wrangler.toml](../cred-hub-cf/wrangler.toml) 的 `[vars]` 段**完全同构**——`CRED_HUB_*` 同名键、字符串值，两版本可用同一份配置内容。示例见 [cred-hub.toml.example](cred-hub.toml.example)：

```toml
[vars]
CRED_HUB_PORT = "8787"
CRED_HUB_DATA = "data/cred-store.redb"
CRED_HUB_RETENTION_MS = "604800000"
CRED_HUB_MAX_CRED_BYTES = "2048"
CRED_HUB_CORS = "1"
# CRED_HUB_ADMIN_TOKEN = "..."
```

各配置项（`[vars]` 键名与环境变量同名，取值语义与 CF 版一致）：
...
> ⚠️ `cred-hub.toml` 里若配置了 `CRED_HUB_ADMIN_TOKEN` 即属敏感文件，已加入 `.gitignore`，请勿提交进 Git。
