...
通过 `/mz/share-mgr.js` 的 `buildOfficialRunUrl` 生成官方应用 HTTP 渠道分享 URL（`?app=cloud-drive&host=<serverUserId>`），客户从 `/official-apps/cloud-drive/` 同源拉取安装，无需发布者保持在线；`host` 参数由 run-app 透传给应用，用于 P2P 客户端通信。
