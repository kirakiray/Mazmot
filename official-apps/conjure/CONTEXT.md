...
**流程**（双端共享协议 `/bridge/proto.js`：服务 ID（`conjure-preview`/`conjure-bridge`/`conjure-agent`）、消息类型、`sanitizeAppName`/`validateRelPath` 守卫、`chunkText` 字节分片（预算 32KB；**切点按 code point 对齐**——历史缺陷是把 emoji 的代理对劈开，孤立代理项经 TextEncoder/E2EE 变 U+FFFD 不可逆损坏、WebKit 传输链直接丢消息，曾长期误诊为尺寸问题。代理对修复后 CI 又暴露独立根因：双端经并行竞速分裂到不同区域的公网中继（实测 jp1↔us1）时，跨区转发的 91KB 级大帧（64KB 文本经 JSON+E2EE+base64 放大）会整窗丢失——中继报 delivered:true 但对端从未收到，小消息全部正常；32KB 预算（≈45KB 帧）避让该故障区间，治本见下方 ensureServerConnected）、`sha256Hex`+`buildManifest`（文件指纹清单）、`createReliableLink`（信封 + ACK + 超时重发 + msgId 去重 + 串行队列，单条 ≤128KB；ACK 等待与重试次数均按载荷字节放宽——每 16KB 等待 +1s、每 32KB 追加一次重试（上限 +5）：noneos 文档明确通道切换期间消息可能丢失，大分片需更长总重试窗口等通道回稳；`onOffline` 钩子带 reason——capped（sendTo 超时无响应，连接疑似僵尸）时消费方应硬重置连接、not-delivered 软检查即可，且**每次主动重连授予 +1 重试额度（封顶基础 +2）并缩短重连后等待**——曾实测「最后一次尝试才触发重连但额度已尽直接拒绝」的重连白做；`sendCapMs` 单次 sendTo 8s 封顶——noneos 内部命令超时可长达 45s，不封顶会拖垮重试节奏；`onEvent` 事件日志钩子——attempt/sent/send-capped/reconnect/ack/duplicate/rejected 全链路可追踪，默认 console.debug，测试收进失败 content）；`enableServerAutoReconnect(user)`——**noneos 服务器自动重连默认关闭**，CI 中继长连接被杀后用户会永远停在 offline（实测 WebKit CI ACK timeout 的根因），各端 getUser 后必须立即开启（baseDelay 1s、退避至 15s）；`ensureServerConnected(user, {hard})`——双端**收敛到排序首位的同一台中继**（断开非首选连接；localhost 源下默认列表的 `ws://localhost:8081` 排序必胜，CI 用 test-bin/ 本地信令服务器即由此闭环），hard 时首选也 disconnect+connect 重建治僵尸）、`createFileAssembler` 分片拼装；接收端核心在 `/bridge/receiver.js`）：
