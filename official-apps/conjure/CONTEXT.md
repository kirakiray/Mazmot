...
**流程**（双端共享协议 `/bridge/proto.js`：服务 ID（`conjure-preview`/`conjure-bridge`/`conjure-agent`）、消息类型、`sanitizeAppName`/`validateRelPath` 守卫、`chunkText` 字节分片（预算 64KB；**切点按 code point 对齐**——历史缺陷是把 emoji 的代理对劈开，孤立代理项经 TextEncoder/E2EE 变 U+FFFD 不可逆损坏、WebKit 传输链直接丢消息，曾长期误诊为尺寸问题）、`sha256Hex`+`buildManifest`（文件指纹清单）、`createReliableLink`（信封 + ACK + 超时重发 + msgId 去重 + 串行队列，单条 ≤128KB；ACK 等待与重试次数均按载荷字节放宽——每 16KB 等待 +1s、每 32KB 追加一次重试（上限 +5）：noneos 文档明确通道切换期间消息可能丢失，大分片需更长总重试窗口等通道回稳；另支持 `onOffline` 钩子——发送 offline 时先回调消费方重连信令服务器再重试；`sendCapMs` 单次 sendTo 8s 封顶——noneos 内部命令超时可长达 45s，不封顶会拖垮重试节奏；`onEvent` 事件日志钩子——attempt/sent/send-capped/reconnect/ack/duplicate/rejected 全链路可追踪，默认 console.debug，测试收进失败 content）；`enableServerAutoReconnect(user)`——**noneos 服务器自动重连默认关闭**，CI 中继长连接被杀后用户会永远停在 offline（实测 WebKit CI ACK timeout 的根因），各端 getUser 后必须立即开启（baseDelay 1s、退避至 15s））、`createFileAssembler` 分片拼装；接收端核心在 `/bridge/receiver.js`）：
...
- **协议**（`/bridge/proto.js`）：conjure → 代理 `{type:"dbg", cmd, args, reqId}`；结果回传 `{type:"dbg-chunk", reqId, seq, total, text}`（大结果按 `chunkText` 同 64KB 预算分片）+ `{type:"dbg-result", reqId, ok, result|error, chunks?, meta?}` 汇总；`buildDbgResultMessages` / `createDbgCollector` 为双端构造/聚合实现（纯逻辑单测覆盖）。
