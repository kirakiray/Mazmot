...
2. `remote-preview.js` 的 `openRemotePreview`：`getUser("conjure-preview")` → 注册 `conjure-preview` 服务（接收 bridge 的 hello/done）→ 等信令服务器连接 → `window.open` 新标签 `?u=<userId>` → 等 bridge hello（120s 兜底，首次访问 bridge 需装 Core）→ `connectUser` 建链 → 串行推送 `app-begin` / 每文件 `file`（大文件按 seq/total 分片）/ `app-end` → 等 done。
3. bridge 页（`bridge/bridge.html`，Core 引导入口、资源走 jsdelivr 完整 URL）：nos-version 自装 Core → `getUser("conjure-preview")` → 注册 `conjure-bridge` 服务 → 连接 `?u=` 指定的 conjure 用户 → hello → 收文件经 `bridge/receiver.js` 的 `createPreviewReceiver`（消息串行化处理，app-begin 清旧目录重建）写入本域 VFS `conjure-apps/<name>/client/`（路径经 `validateRelPath` 复检）→ app-end 后回 done；跳转前经 `waitUrlReady` 轮询 URL 可访问（首装 Core 后 SW 激活存在窗口期，立即跳转会 404）再跳 `/$conjure-apps/<name>/client/index.html`。
...
- 隔离预览协议层测试在 [bridge/test/proto.sb.html](../../bridge/test/proto.sb.html)：`chunkText` 字节分片（ASCII/中文/emoji 混合、超预算切分、无损还原）、`buildFileMessages` seq/total、`createFileAssembler` 乱序收齐与非法 seq、`createReliableLink`（ACK resolve、串行保序、重复去重且先回 ACK、失败重发）、`assertSendable` 超限、`sanitizeAppName` / `validateRelPath` 守卫；全链路集成测试在 [bridge/test/preview-flow.sb.html](../../bridge/test/preview-flow.sb.html)（需 Core 已就绪，单页内两个真实 LocalUser 模拟双端）：hello → 分片推送（含 >96KB 大文件）→ receiver 落盘 → `/$conjure-apps/...` URL 可访问且内容一致、重复推送覆盖旧文件、路径逃逸拦截、`waitUrlReady` 就绪/超时行为
