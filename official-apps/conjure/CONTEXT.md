...
**流程**（双端共享协议 `/bridge/proto.js`：服务 ID（`conjure-preview`/`conjure-bridge`/`conjure-agent`）、消息类型、`sanitizeAppName`/`validateRelPath` 守卫、`chunkText` 字节分片（预算 32KB——CI 等 UDP 受限环境只能走信令中继，64KB 级大分片实测曾触发中继连接掉线，缩小后任何放大系数下都远离服务端 256KB 硬限）、`sha256Hex`+`buildManifest`（文件指纹清单）、`createReliableLink`（信封 + ACK + 超时重发 + msgId 去重 + 串行队列，单条 ≤128KB；ACK 等待与重试次数均按载荷字节放宽——每 16KB 等待 +1s、每 32KB 追加一次重试（上限 +5）：noneos 文档明确通道切换期间消息可能丢失，大分片需更长总重试窗口等通道回稳；另支持 `onOffline` 钩子——发送 offline 时先回调消费方重连信令服务器再重试）；`enableServerAutoReconnect(user)`——**noneos 服务器自动重连默认关闭**，CI 中继长连接被杀后用户会永远停在 offline（实测 WebKit CI ACK timeout 的根因），各端 getUser 后必须立即开启（baseDelay 1s、退避至 15s））、`createFileAssembler` 分片拼装；接收端核心在 `/bridge/receiver.js`）：
