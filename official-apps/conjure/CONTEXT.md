...
- **AI Agent**：`/mz/ai/main.js`（`getAssistant` / `getApiKeys` / `onApiKeysChange`）+ `/mz/ai/chain/main.js` 的 `createAgent` + `tool` + storage 版 checkpointer；assistant 选择：用户在输入区 select 手动指定的 key 优先（`activeKeyId`，deepseek 供应商配 `deepseek-flash` 模型名，其余跟随供应商默认），模型同理可手动指定（`activeModelId`，须属于当前 key 的供应商，选项来自 `MODEL_OPTIONS` 映射）；未指定时自动——优先 DeepSeek（`deepseek-flash`）否则 `getAssistant()` 随机负载均衡；未配置 Key 时发送报错提示去「AI 密钥管理器」。key 列表镜像在 state `apiKeys`，`onApiKeysChange` 实时刷新，选中项被删/禁用自动回退自动模式；Key 切换器、模型选择器与上下文窗口选择全部收在输入区左下角圆圈的悬停气泡里（`.ctx-pop`，三行「模型 / Key / 窗口」，纯 CSS hover 展示，不占控件行；模型选项跟随选中 Key 的供应商，自动 Key 时禁用）。圆圈**常驻**并带扩展透明热区（`.ctx-anchor` 四周 padding + `.ctx-pop::after` 隐形过渡带，斜向移动 hover 不断链）：会话无上下文占用时显示「🤖 AI」，作为选择入口（点击无动作）；有占用后显示进度百分比、点击压缩（`handleCompress` 对空占用直接跳过）
