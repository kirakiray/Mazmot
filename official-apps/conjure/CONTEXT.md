...
**流程**（双端共享协议 `/bridge/proto.js`：服务 ID、消息类型、`sanitizeAppName`/`validateRelPath` 守卫、`chunkText` 字节分片、`createReliableLink`（信封 + ACK + 超时重发 + msgId 去重 + 串行队列，单条 ≤128KB）、`createFileAssembler` 分片拼装）：
...
2. `remote-preview.js` 的 `openRemotePreview`：`getUser("conjure-preview")` → 注册 `conjure-preview` 服务（接收 bridge 的 hello / sync-diff / done）→ 等信令服务器连接 → `window.open` 新标签 `?u=<userId>` → 等 bridge hello（120s 兜底，首次访问 bridge 需装 Core）→ `connectUser` 建链 → **增量同步**：发 `sync-check`（`buildManifest` 的「路径 + sha256」清单，每条约 80 字节），bridge 比对本域 VFS 后回 `sync-diff`（需重传的 path 列表）；conjure 只推送差异文件（部分命中 → `app-begin` 带 `wipe:false` 覆盖写入，全部命中/全新 → `wipe` 全量清目录重建），无差异时跳过推送直接等 done；比对超时（30s）/异常回退全量。
3. bridge 页（`bridge/bridge.html`，Core 引导入口、资源走 jsdelivr 完整 URL）：nos-version 自装 Core → `getUser("conjure-preview")` → 注册 `conjure-bridge` 服务 → 连接 `?u=` 指定的 conjure 用户 → hello → 收 `sync-check` 经 `bridge/receiver.js` 的 `createPreviewReceiver`（消息串行化处理）对本域 VFS 逐文件 sha256 比对并回 `sync-diff`，已最新则直接进入启动流程；否则按 `app-begin` 的 wipe 语义（全量清目录重建 / 增量覆盖）写入 `conjure-apps/<name>/client/`（路径经 `validateRelPath` 复检）→ app-end 后回 done；跳转前经 `waitUrlReady` 轮询 URL 可访问（首装 Core 后 SW 激活存在窗口期，立即跳转会 404）再跳 `/$conjure-apps/<name>/client/index.html`。
