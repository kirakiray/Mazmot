...
**流程**（双端共享协议 `/bridge/proto.js`：服务 ID（`conjure-preview`/`conjure-bridge`/`conjure-agent`）、消息类型、`sanitizeAppName`/`validateRelPath` 守卫、`chunkText` 字节分片（预算 64KB，E2EE + JSON 放大后仍远离服务端 256KB 硬限）、`sha256Hex`+`buildManifest`（文件指纹清单）、`createReliableLink`（信封 + ACK + 超时重发 + msgId 去重 + 串行队列，单条 ≤128KB；ACK 等待与重试次数均按载荷字节放宽——每 16KB 等待 +1s、每 32KB 追加一次重试（上限 +5）：noneos 文档明确通道切换期间消息可能丢失，大分片需更长总重试窗口等通道回稳）、`createFileAssembler` 分片拼装；接收端核心在 `/bridge/receiver.js`）：
