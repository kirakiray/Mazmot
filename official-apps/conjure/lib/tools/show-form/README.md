...
- `self-test.js` — 内置测试模组：用 `lib/test-space` 的 `defineVisualSelfTest` 声明用例（基座的用途、API 与 ofa.js 相关坑见 [../../test-space/README.md](../../test-space/README.md)），导出 `testPlan`（用例名清单）与 `runSelfTest(onCase)`（返回 `{ ok, cases: [{ name, pass, info }] }`）。覆盖插件层（包结构 / 参数清洗 / 提交 / 取消 / 环境兜底）与组件层（控件渲染 / required 拦截 / 事件冒泡 / 只读回填 / XSS 转义；组件或 senti 控件未注册的环境自动跳过）。
