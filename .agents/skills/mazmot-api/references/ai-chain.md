...
`tools` 可传 `() => array`（支持 async），每轮循环求值一次，可在会话中途增删工具：
...
// 第一轮：无工具可用
await agent.chat({ messages: [{ role: "user", content: "上海天气怎么样？" }] });

// 中途加入天气工具
toolPool.push(
  tool(
    async ({ city }) => `【${city}】晴，26℃`,
    { name: "get_weather", description: "查询城市天气", schema: { city: { type: "string" } } },
  ),
);

// 第二轮：模型就能调用 get_weather 了
const result = await agent.chat({ messages: [{ role: "user", content: "再查一次上海天气" }] });
```

> 数组形式是创建时的固定引用：往同一个数组里 push 也会在后续轮次生效（循环每轮读取其内容），但换新数组赋值（`myTools = [...]`）无效；要表达明确的动态语义，建议用函数形式。
...
- **工具循环检测**（`isToolLoop`）：追踪已执行工具调用的签名（工具名 + 按 key 深排序的规范化参数）——连续 4 次完全相同、或最近 8 次里只有 ≤2 种签名（A,B,A,B… 窄循环）判定为真循环。命中后不抛错，而是注入一条 user 提醒并**收起后续轮次的工具**，逼模型基于已有信息收尾，回合优雅结束。参数递增、与不同调用交织的合法重复不会误伤。
...
| `thinking` / `reasoningEffort` / `thinkingKeep` | - | 同名参数直传，语义见 [ai.md 的 chat 参数表](./ai.md#chatoptions) |
| `tools` | array / `() => array` `[]` | `tool()` 定义的工具列表，可为空（退化为普通对话）；传函数时每轮求值（支持 async），可在会话中途动态增删 |
