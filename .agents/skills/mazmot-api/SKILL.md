...
```javascript
const {
  issueCert,          // ({ subject, role, expire, extras }) → 签发并入库
  claimCert,          // (holderId, role, { issuerId }) → 在线拉取（唯一送达方式）；
                      //   holderId=保管人（个人=签发者本人；组织证书=托管的创建者），
                      //   issuerId=实际签发者（组织证书传组织 ID）
  revokeCert,         // (certId) → 本地删除（core 无软删除）
  lookupProfile,      // (userId, { force }) → 用户卡片查询
  verifyProfileCard,  // (profile, targetId) → 卡片验签（勿用 user.verify 验别人的）
  appendIssueHistory, listIssued, mergeIssuedView,   // 签发历史 / 吊销留痕
  buildRef, parseRef, buildChain, collectChainFields, certFingerprint,  // 链式引用纯函数
...
## 11. 云盘套件共享层 —— `/mz/cloud-drive/*`

云盘服务器（`official-apps/cloud-drive-server`）与客户端（`official-apps/cloud-drive-client`）共用的协议、可靠传输与业务核心。
...
### `reliable.js` —— ReliableChannel（纯模块，可注入传输层）
...
- **客户端必须 registerService 同名 appId**：服务器应答经 `sendToService` 回投，客户端不注册服务则应答 `no_receiver`，表现为「请求超时: ping」。
- **页面模块内禁止动态 `import("/gh/...")`**：ofa.js 把页面脚本编译为 data: URL，无 base 无法解析根路径；senti-ui 的 `alert/confirm/prompt/toast` 一律在模块初始化期 `await load("/gh/...")` 预载到闭包。
- **ofa.js 页面保留名**：proto 上 `refresh`、data 上 `entries` 均被占用（注册失败整页白屏，console 报「已被占用」）；用 `refreshAll` / `fileItems` 等业务命名。
- **页面跳转用相对页面路径**：`this.goto("./files.html")`；`this.app.goto("//files")` 会被当前 ofa.js 判为跨域跳转报「必须设置 access 函数」。
