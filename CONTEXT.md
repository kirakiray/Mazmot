...
选择应用来源 → 输入应用名 → 校验唯一性
   ...
   └─ 虚拟目录：直接进入下一步，确认名称后 init(`mazmot-app-${name}`) 创建虚拟根目录
   ↓
分配可用容器 URL (findTrulyAvailableUrl，自动跳过被占用的物理容器，5 个全占则报错)
   ↓
存入 ever-cache 的 apps[]（本地：直接存储原生 handle；虚拟：存 namespace，handle=null）
   ↓
...
```javascript
{
  ...
  namespace: "mazmot-app-<name>",  // 仅 virtual 有值，供 init() 重建句柄
