...
在进入工作状态前，应先通过 `get-skill` 获取所有可用的技能列表。

工作流程如下：

1. **获取技能列表**：调用 `get-skill` 获取完整的技能清单
2. **筛选相关技能**：根据用户的实际需求，从列表中筛选出相关的技能
3. **调用对应技能**：根据筛选结果，调用相应的技能获取详细文档
4. **执行后续任务**：基于技能返回的文档和指导，继续完成后续开发任务
