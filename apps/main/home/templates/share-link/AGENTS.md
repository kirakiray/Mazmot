...
2. **涉及 Mazmot / NoneOS Core API 必读**：使用 `app.json`、分享发布（`publishApp` / `appParams`）、文件系统（`init` / `rootDir.get`）、应用运行等 API 前，必须先查阅 `mazmot-api` 技能文档（Mazmot 上层编排）与 `noneos-core-docs` 技能文档（底层 fs / user），按官方 API 与约定调用，禁止凭记忆写。本模板核心 API 的调用清单与关键约定见 [CONTEXT.md](CONTEXT.md)。
