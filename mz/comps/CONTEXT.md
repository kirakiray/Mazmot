...
- **核心能力**：
  ...
  - `auto-save-id` 属性：自动将位置、尺寸、focus 状态持久化到 `getStorage("mazmot-rnd-box")`（key 规则 `mazmot:rnd-box:${autoSaveId}`）。刷新后按比例恢复，防止越界。
...
- 涉及本地持久化使用 `/nos/storage/main.js`（按业务用 `getStorage(<id>)` 划分空间），禁止裸用 `localStorage`。
