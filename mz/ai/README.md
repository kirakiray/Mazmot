...
> DeepSeek 官方按 `low` / `high` / `max` 三档执行思考强度；请求还可传 `minimal` / `medium` / `xhigh` / `ultra`，由官方自动映射（minimal/low → low，medium/high/xhigh → high，max/ultra → max）。本库原样透传 `reasoningEffort`，不做本地映射。
...
GLM（智谱 bigmodel.cn）按模型版本分两种思考控制方式：
