...
export const SHARE_NAMESPACE = "default";
 ...
 *
 * `app` 为官方应用专用保留参数：官方应用通过同源 `/official-apps/` 下发，
 * 链接形如 `?app=<officialId>`，不会带给应用本身。
 */
export const RESERVED_SHARE_KEYS = ["u", "h", "app"];
 ...
 * 构造官方应用的"一键安装"分享链接。
 *
 * 官方应用作为同源公共资源（位于 `/official-apps/<id>/`），不需要 P2P 分发，
 * 链接只携带 `app=<officialId>`，接收端通过 HTTP 直接拉取并安装。
 *
 * @param {string} origin - 当前域名（location.origin）
 * @param {string} officialId - 官方应用 ID（对应 official-apps/<id>/ 目录名）
 * @param {Object<string,string|number>} [appParams] - 应用业务参数（可选）
 * @returns {string}
 */
export function buildOfficialRunUrl(origin, officialId, appParams) {
  let url = `${origin}/apps/run-app/?app=${encodeURIComponent(officialId)}`;
  if (appParams && typeof appParams === "object") {
    const sp = new URLSearchParams();
    for (const [key, value] of Object.entries(appParams)) {
      if (value !== undefined && value !== null) {
 ...
 *
 * 同时识别 P2P（u/h）与官方应用（app）两种格式。若两者都存在，优先按官方应用处理。
 * @param {string} search - location.search（带或不带 "?" 均可）
 * @returns {{ kind:"p2p", userId: string, payloadHash: string } | { kind:"official", officialId: string } | null}
 */
export function parseShareUrl(search) {
  if (!search) return null;
  const query = search.startsWith("?") ? search.slice(1) : search;
  const params = new URLSearchParams(query);
  // 官方应用优先：app= 参数存在即按官方应用处理（即使同时携带 u/h）
  const app = params.get("app");
  if (app) return { kind: "official", officialId: app };
  const u = params.get("u");
  const h = params.get("h");
  if (!u || !h) return null;
  return { kind: "p2p", userId: u, payloadHash: h };
}
