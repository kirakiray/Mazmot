...
1. **对话式能力**：通过传统的请求-响应模式与用户交互，所有数据通过 `<template>` 标签传递
2. **页面交互能力**：通过 ofajs 的 page 模块提供可视化界面，用户通过定制化的 UI 完成交互后，结果通过 `<template>` 标签返回

**识别方式**：查看能力文档顶部的 YAML 元数据，如果包含 `page` 字段，则为页面交互能力。例如：
...
页面交互能力提供更直观的用户界面，适合需要复杂输入、多选项、表单填写等场景。
