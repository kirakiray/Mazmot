...
使用 `cap-request` 标签发起调用。**禁止用 markdown 代码块（```）包裹，否则会被当作代码展示而非实际调用。** 格式为 **JSON 数组**，列表中每一项代表一个能力调用：
...
- **必须先获取详细文档再调用**：筛选出有用能力后，必须先调用 `get-capability` 获取其详细使用方法，然后才能调用该能力
- **禁止猜测能力用法**：不要根据能力名称猜测参数格式，必须查阅文档后按文档示例调用
- `name` 参数是数组（复数），一次可查询多个能力的详细使用方式
